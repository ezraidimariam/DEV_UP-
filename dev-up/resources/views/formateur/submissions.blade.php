...
<head>
    ...
    <title>DEV↑UP - Submissions Review</title>
...
<body>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                ...
                <div class="hidden md:flex items-center space-x-8">
                    ...
                    <a href="{{ route('formateur.submissions') }}" class="nav-link active text-white">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <!-- Header -->
        <div class="mb-8">
            <h1 class="brand-font text-4xl font-bold gradient-text mb-2">Submissions Review</h1>
            <p class="text-gray-400">Review and grade student submissions</p>
        </div>
        ...
        <div class="glass-card rounded-2xl">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center">
                                            <div class="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center mr-3">
                                                <span class="text-white text-sm font-semibold">{{ strtoupper(substr($submission->user->name, 0, 1)) }}</span>
                                            </div>
                                            <div>
                                                <div class="text-sm font-medium text-white">{{ $submission->user->name }}</div>
                                                <div class="text-xs text-gray-400">Level {{ $submission->user->level }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div>
                                            <div class="text-sm font-medium text-white">{{ $submission->challenge->title }}</div>
                                            <div class="text-xs text-gray-400">{{ $submission->challenge->difficulty }} • {{ $submission->challenge->points }} pts</div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="text-sm text-gray-300">{{ $submission->created_at->format('M j, Y') }}</div>
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <p class="text-gray-400">No submissions pending review at the moment.</p>
                </div>
            @endif
        </div>
    </main>
